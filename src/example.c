...
main(int argc, char** argv)
{
	...
	if (Holo_Init(settings))
	{
		...
		if (webcam_names != 0)
		{
			webcam_reader = HoloCameraReader_Create(webcam_names[0].symbolic_name, WIDTH, HEIGHT, FPS);
		}

		Holo_Cam* cam = HoloCam_Create(L"HoloCam Example", WIDTH, HEIGHT, FPS, PORT);
				...
				for (unsigned int j = 0; j < HEIGHT; ++j)
				{
					for (unsigned int i = 0; i < WIDTH; ++i)
					{
						...
						*/
						...
						// Only red channel example
						rgb_frame[j*WIDTH + i] = color & 0xFF0000;
					}
				}
